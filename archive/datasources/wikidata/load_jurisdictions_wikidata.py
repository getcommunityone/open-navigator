...
def _postgres_fips_gnis_map_has_rows(conn) -> bool:
    ...
    cur = conn.cursor()
    try:
        cur.execute("SELECT 1 FROM bronze.bronze_jurisdiction_wikidata_fips_gnis_map LIMIT 1")
...
def _wikidata_skip_municipality_bulk_wdqs(
    ...
    (env flag, parquet file, ``bronze.bronze_jurisdiction_wikidata_fips_gnis_map``, or most bronze rows have Q-ids).
    """
