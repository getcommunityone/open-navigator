...
async function fetchWikipediaArticle(title: string): Promise<string> {
  ...
  // Wikimedia requires a descriptive User-Agent; without one it returns an HTML
  // block page (parsed as JSON -> "Unexpected token '<'"), especially from cloud IPs.
  const res = await fetch(url, {
    headers: {
      'User-Agent':
        'open-navigator-rag-chat/1.0 (https://github.com/getcommunityone/open-navigator; johnbowyer@getcommunityone.onmicrosoft.com)',
