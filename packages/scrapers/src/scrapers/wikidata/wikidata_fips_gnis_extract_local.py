...
Usage:
    ...
    .venv/bin/python packages/scrapers/src/scrapers/wikidata/wikidata_fips_gnis_extract_local.py

    # Ignore checkpoint and scan from the beginning
    .venv/bin/python packages/scrapers/src/scrapers/wikidata/wikidata_fips_gnis_extract_local.py --no-resume

    # Save to Postgres instead of Parquet
    .venv/bin/python packages/scrapers/src/scrapers/wikidata/wikidata_fips_gnis_extract_local.py --postgres

    # Quick test — stop after 500k entities (~2-3 min)
    .venv/bin/python packages/scrapers/src/scrapers/wikidata/wikidata_fips_gnis_extract_local.py --limit 500000

    # Stream from Wikimedia over HTTP (no local .bz2)
    .venv/bin/python packages/scrapers/src/scrapers/wikidata/wikidata_fips_gnis_extract_local.py --remote
