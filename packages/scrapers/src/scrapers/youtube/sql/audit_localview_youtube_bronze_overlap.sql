...
--   bronze.bronze_event_youtube   — YouTube API / loader rows (video_id).
--
-- A gap between totals is normal: not every LocalView meeting is re-ingested into
-- bronze_event_youtube, and YouTube bronze may include videos never in LocalView.
--
-- Why overlap row-count can be ~1%: LocalView is a wide catalog (many meetings / many
-- places). bronze_event_youtube only has rows you actually pulled via the YouTube loader
-- for configured channels. Fix = ingest more videos (same video_id) into bronze_event_youtube,
...
SELECT
    (SELECT COUNT(*)::bigint FROM bronze.bronze_event_youtube) AS n_youtube_all_states,
    (SELECT COUNT(*)::bigint FROM bronze.bronze_event_youtube y WHERE y.state_code = trim(upper(:'one_state'))) AS n_youtube_this_state,
...
FROM bronze.bronze_events_localview lv
LEFT JOIN bronze.bronze_event_youtube y ON BTRIM(y.video_id) = BTRIM(lv.datasource_id)
WHERE lv.datasource = 'localview'
  AND lv.state_code = trim(upper(:'one_state'));
...
\echo ''
\echo '========== LocalView rows: video_url has watch?v= but id not in bronze_event_youtube (sample) =========='
