...
Usage:
    ...
    python packages/scrapers/src/scrapers/youtube/load_youtube_events_to_postgres.py --states AL,MA,WI
    ...
    python packages/scrapers/src/scrapers/youtube/load_youtube_events_to_postgres.py --days 30
    
    # Limit videos per channel
    python packages/scrapers/src/scrapers/youtube/load_youtube_events_to_postgres.py --max-videos 10
    
    # Skip transcript fetching (faster)
    python packages/scrapers/src/scrapers/youtube/load_youtube_events_to_postgres.py --skip-transcripts
    
    # Fetch text transcripts only (no VTT downloads, faster and cleaner)
    python packages/scrapers/src/scrapers/youtube/load_youtube_events_to_postgres.py --text-transcripts-only
...
from scrapers.youtube.scrape_youtube_channels import MunicipalYouTubeScraper
...
class YouTubeEventsLoader:
    ...
    def _ytdlp_transcript_opts(
        ...
        *,
        ...
        from scrapers.youtube.download_audio_to_drive import _yt_dlp_youtube_ejs_opts
    ...
    def _ytdlp_extract_info_with_proxy_fallback(
        ...
        *,
        ...
        from scrapers.youtube.transcript_api_client import ytdlp_proxy_is_webshare
    ...
    def run(
        ...
        *,
        ...
        if not self.fetch_transcripts:
            ...
            logger.info(f"  python packages/scrapers/src/scrapers/youtube/backfill_transcripts.py --states {','.join(states_filter) if states_filter else 'AL,GA,IN,MA,MT,WA,WI'} --limit 100")
            logger.info("")
            ...
            logger.info(f"  python packages/scrapers/src/scrapers/youtube/backfill_transcripts.py --states {','.join(states_filter) if states_filter else 'AL,GA,IN,MA,MT,WA,WI'}")
