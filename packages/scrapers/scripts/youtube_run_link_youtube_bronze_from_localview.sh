...
#
# Usage (repo root):
#   ./packages/scrapers/src/scrapers/youtube/run_link_youtube_bronze_from_localview.sh
#   ./packages/scrapers/src/scrapers/youtube/run_link_youtube_bronze_from_localview.sh --apply
...
if [[ "${1:-}" == "--apply" ]]; then
  SQL="${ROOT}/packages/scrapers/src/scrapers/youtube/link_youtube_bronze_from_localview_apply.sql"
  echo "==> APPLY: ${SQL}"
else
  SQL="${ROOT}/packages/scrapers/src/scrapers/youtube/link_youtube_bronze_from_localview_preview.sql"
  echo "==> PREVIEW: ${SQL}"
