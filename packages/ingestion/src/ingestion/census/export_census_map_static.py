...
Usage (from repo root, project venv):
  ...
  .venv/bin/python packages/scrapers/src/scrapers/census/export_census_map_static.py

  # After caching ACS county tables (default download = all U.S. counties):
  .venv/bin/python packages/scrapers/src/scrapers/census/download_census_acs_data.py --year 2022

  # City/places for a state (repeat per state you want in the SPA):
  .venv/bin/python packages/scrapers/src/scrapers/census/download_census_acs_data.py --geography place --state 01 --year 2022

  .venv/bin/python packages/scrapers/src/scrapers/census/export_census_map_static.py \\
      --all-years --place-states 01

  # Place (city) layer for every state + DC + PR (large: needs place ACS parquets per state):
  .venv/bin/python packages/scrapers/src/scrapers/census/export_census_map_static.py --all-place-states --year 2022

  # Optionally fetch missing county / place parquets (same tables as METRICS) before export:
  .venv/bin/python packages/scrapers/src/scrapers/census/export_census_map_static.py --fetch --place-states 01
"""
