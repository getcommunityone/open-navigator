...
#
# Examples:
#   ./scripts/accessibility/run_verapdf_scan.sh --state AL
#   VERAPDF_FLAVOURS=ua1,ua2 ./scripts/accessibility/run_verapdf_scan.sh --limit 100
#   ./scripts/accessibility/run_verapdf_scan.sh --manifest data/cache/accessibility/pdf-urls.json --skip-discover
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]:-$0}")" && pwd)"
ROOT="$SCRIPT_DIR"
while [[ "$ROOT" != "/" && ! -f "$ROOT/scripts/accessibility/export_pdf_urls.py" ]]; do
  ROOT="$(dirname "$ROOT")"
done
cd "$ROOT"
...
if [[ "$SKIP_DISCOVER" -eq 0 ]]; then
  DISC_ARGS=( -m scripts.accessibility.export_pdf_urls --out "$MANIFEST" --max-pdfs-per-site "$MAX_PDFS" )
...
SCAN_ARGS=( -m scripts.accessibility.run_verapdf_scan --manifest "$MANIFEST" --out "$OUT" )
[[ -n "$LIMIT" ]] && SCAN_ARGS+=( --limit "$LIMIT" )
[[ "$OFFSET" -gt 0 ]] && SCAN_ARGS+=( --offset "$OFFSET" )
"$PY" "${SCAN_ARGS[@]}"

if [[ "$SKIP_PERSIST" -eq 0 ]]; then
  "$PY" -m scripts.accessibility.persist_verapdf_results --input "$OUT" --ensure-ddl
fi
