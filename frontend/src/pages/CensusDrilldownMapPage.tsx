  ...
  return (
    <div className="mx-auto w-full min-w-0 space-y-2.5">
      ...
      <div className="flex flex-col gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 shadow-sm sm:flex-row sm:items-center">
        ...
        {/* Desktop (xl+) selects metrics in the persistent sidebar browser; the
            dropdown is the small-screen affordance where the rail is hidden. */}
        <div className="min-w-0 flex-1 sm:max-w-sm xl:hidden">
          <MetricPicker
      ...
      >
        {/* left unit: display rail + persistent metric browser */}
        <div className="hidden xl:flex xl:sticky xl:top-4 xl:h-[calc(100vh-2rem)] xl:gap-2">
          <CensusMapLeftRail
            ...
            onToggleMetricPanel={() => setMetricPanelOpen((v) => !v)}
            metricPanelOpen={metricPanelOpen}
          />
          {metricPanelOpen ? (
            <CensusMetricBrowserPanel
              metrics={selectableMetrics}
              metricSlug={metricSlug}
              metricFullHelp={metricFullHelp}
              onPick={onMetricChange}
              onCollapse={() => setMetricPanelOpen(false)}
            />
          ) : null}
        </div>
