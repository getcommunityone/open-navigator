#!/usr/bin/env python3
"""Move flat gemini_transcript_policy folders into {state}/{type}/{jurisdiction_id}/."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--jurisdiction-id",
        default="",
        help="Move only this folder (default: all legacy top-level jurisdiction dirs)",
    )
    parser.add_argument("--dry-run", action="store_true")
    args = parser.parse_args()

    stats = migrate_policy_cache_geography(
    ...
    if args.dry_run:
        logger.info("Dry run — no folders moved")
