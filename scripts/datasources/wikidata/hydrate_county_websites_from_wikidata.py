#!/usr/bin/env python3
"""
Hydrate ``official_website`` on ``bronze.bronze_jurisdictions_counties_wikidata`` from Wikidata.

Uses ``wbgetentities`` (no bulk WDQS when parquet is available) for rows that already have a
``wikidata_id`` but no website. Sets ``official_website_updated_at`` when a website is written.

Prerequisites:
  - Census + bronze county rows (``ensure_bronze_jurisdictions_cloud.py``)
  - ``wikidata_id`` on rows (parquet apply or prior loader run)

Examples::

  .venv/bin/python scripts/datasources/wikidata/hydrate_county_websites_from_wikidata.py --states AL
  .venv/bin/python scripts/datasources/wikidata/hydrate_county_websites_from_wikidata.py --priority-states
  .venv/bin/python scripts/datasources/wikidata/hydrate_county_websites_from_wikidata.py --all-us-states
"""
...
from scripts.datasources.wikidata.load_jurisdictions_wikidata import (  # noqa: E402
    DATABASE_URL,
    PRIORITY_STATES,
    STATE_MAP,
    CheckpointManager,
...
COUNTY_TABLE = "bronze.bronze_jurisdictions_counties_wikidata"


def _snapshot_county_websites(conn, states: List[str]) -> Dict[str, Dict[str, int]]:
...
def _apply_hydrate_env() -> None:
    ...
    os.environ["WIKIDATA_LOAD_RETRY_COUNTY_GAPS"] = "0"
...
async def _run_states(
    ...
    *,
    ...
    try:
        for us in states:
            logger.info(f"=== Hydrate county websites: {us} ===")
            before = loader._county_website_stats(us)
            await loader.load_state(us, ["county"], checkpoint)
            after = loader._county_website_stats(us)
            per_state[us] = {
                "before": before,
                "after": after,
                "websites_added": max(
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Hydrate official_website on bronze counties_wikidata via Wikidata API"
    )
    ...
    ap.add_argument(
        ...
        type=Path,
        default=None,
        help="JSON run summary (default: data/logs/county_website_hydrate_<ts>.json)",
    )
    ...
    log_path = args.log_file or (log_dir / f"county_website_hydrate_{stamp}.json")

    conn = psycopg2.connect(db_url)
    try:
        before_all = _snapshot_county_websites(conn, states)
    finally:
        conn.close()
    ...
    conn = psycopg2.connect(db_url)
    try:
        after_all = _snapshot_county_websites(conn, states)
    finally:
        conn.close()
