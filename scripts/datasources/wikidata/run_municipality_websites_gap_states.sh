...
#   ./scripts/datasources/wikidata/run_municipality_websites_gap_states.sh --states VT,WA,WV,WY
...
if [[ "${DISCOVER}" == "1" ]]; then
  STATES="$("$PY" "${ROOT}/scripts/datasources/wikidata/discover_municipality_website_gaps.py" --mode no_wikidata)"
fi
...
from scripts.datasources.wikidata.load_jurisdictions_wikidata import DATABASE_URL, JurisdictionsWikiDataLoader
...
if [[ -f "${PARQUET}" ]]; then
  ...
  "$PY" "${ROOT}/scripts/datasources/wikidata/warm_geography_cache_from_parquet.py" \
...
"$PY" "${ROOT}/scripts/datasources/wikidata/hydrate_municipality_websites_from_wikidata.py" \
  --states "${STATES}" \
  --force
