...
6. **429 Retry-After:** `WIKIDATA_RETRY_AFTER_MAX_SECONDS` (default `45`) caps post-429 sleep (WDQS can send Retry-After in the hundreds of seconds). **`≤0` disables the cap** (~1000s sleeps are possible — avoid for unattended bulk loads; use **`90–180`** or raise throttle / hybrid instead). **`WIKIDATA_HTTPS_PROXY` / `WIKIDATA_HTTP_PROXY`** optionally send WDQS + `w/api.php` through SOCKS5 (e.g. `socks5://127.0.0.1:1080` forwarded from WARP-in-Docker).
