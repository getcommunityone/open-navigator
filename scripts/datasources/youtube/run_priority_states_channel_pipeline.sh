...
#
# Usage (repo root):
#   ./scripts/datasources/youtube/run_priority_states_channel_pipeline.sh
#
# Optional env:
...
for raw_state in "${STATE_LIST[@]}"; do
  ...
  if [[ "$SKIP_DISCOVERY" != "1" ]]; then
    disc_args=(
      scripts/datasources/youtube/load_missing_county_channels.py
      --state "$state"
      --output "$csv_path"
    )
  ...
  if [[ "$SKIP_LOAD" != "1" ]]; then
    load_args=(
      scripts/datasources/youtube/load_channel_candidates_to_catalog.py
      --input "$csv_path"
      --state "$state"
...
if [[ "$SKIP_ABOUT" != "1" ]]; then
  about_args=(
    scripts/datasources/youtube/channel_about_links.py
    --from-bronze-youtube
    --where-null
    --sleep "$ABOUT_SLEEP"
  )
