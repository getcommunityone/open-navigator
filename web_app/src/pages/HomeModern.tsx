...
export default function HomeModern() {
  ...
  const { data: previewResults, isLoading: previewLoading, error: previewError } = useQuery({
    ...
    queryFn: async () => {
      ...
      const params: any = {
        q: debouncedKeyword,
        types: 'causes,leaders,organizations',
        limit: 3
      };
      ...
      try {
        ...
        homeLog('👥 [HomeModern] Leaders:', response.data.results.leaders?.length ?? 0);
                          ...
                          <div className="relative" ref={searchContainerRef}>
                            ...
                            {showSuggestions && !previewError && previewResults && previewResults.total_results > 0 && (
                              <div className="absolute z-50 w-full mt-2 border-2 border-gray-300 rounded-lg shadow-2xl max-h-96 overflow-y-auto" style={{ backgroundColor: '#ffffff' }}>
                                ...
                                {(previewResults.results.leaders?.length ?? 0) > 0 && (
                                  <div className="border-b border-gray-200">
                                    <div className="px-4 py-2 bg-gray-50 flex items-center justify-between">
                                      <div className="flex items-center gap-2">
                                    ...
                                    {previewResults.results.leaders.slice(0, 3).map((result: any, idx: number) => (
                                      <button
                                        key={idx}
                                        type="button"
                                        onMouseDown={(e) => {
