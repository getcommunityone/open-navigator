...
The dbt project includes models to load `stats_aggregates` table with **location-specific trending causes** based on decisions from the last 90 days.
...
# Or manually:
cd dbt_project
dbt run --select stg_bronze_decisions int_trending_causes_by_jurisdiction stats_aggregates
```
