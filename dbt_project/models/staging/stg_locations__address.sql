...
/*
    Staging (MDM address conformance): facility addresses from
    bronze_locations, mapped onto the shared address contract that
    int_addresses__unioned unions. Builds on stg_hifld__location so the
    FIELD_MAP column-name normalization is reused rather than repeated.

    bronze_locations stores the street as one combined string ("4380 BEACH
    STREET"), so the house number is split out here the same way as the parcel
    source. One row per (source_dataset, source_id).
...
source as (
    select * from {{ ref('stg_hifld__location') }}
),

parsed as (
    select
        'bronze_locations'                                     as source_system,
        concat_ws(':', source_dataset, source_id)              as source_pk,
