...
/*
public.event_bill — legislation/bills the AI referenced in analyzed events.
...
src as (
    select * from (
        select *, row_number() over (
            partition by source_event_id_leg_id order by extracted_at desc, title
        ) as _rn
        from src_raw
    ) d where _rn = 1
),
...
select
    md5(s.source_event_id_leg_id)               as event_bill_id,
    s.source_event_id_leg_id                    as extraction_key,
