...
/*
Intermediate: IRS Nonprofits Enriched with ZCTA
...
2. Run with: dbt run --target bronze --select int_nonprofits_irs_with_zcta

Adds 5-digit ZIP Code Tabulation Area (ZCTA) to IRS nonprofit data.
...
This model:
1. Extracts first 5 digits from zip_code (standardizes ZIP+4 to ZIP5)
2. Joins to Census ZCTA lookup table
3. Adds ZCTA geographic boundaries and centroid coordinates

Source tables:
- bronze_organizations_nonprofits_irs (1.95M nonprofits)
- bronze_jurisdictions_postal_codes (33,791 ZCTAs)

Join rate: ~95% of nonprofits have valid 5-digit ZCTAs
*/

WITH irs_nonprofits AS (
    SELECT 
        *,
        -- Extract first 5 digits from zip_code (handles ZIP+4 format)
...
enriched AS (
    ...
    FROM irs_nonprofits AS irs
    LEFT JOIN zcta_lookup AS zcta
        ON irs.zip_code_5 = zcta.zcta
    LEFT JOIN zcta_place_primary AS place
        ON irs.zip_code_5 = place.zcta
    LEFT JOIN zcta_county_primary AS county
        ON irs.zip_code_5 = county.zcta
)
