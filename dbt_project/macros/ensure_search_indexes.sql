...
{#-
    on-run-end safety net for the /search performance indexes.
    ...
    This runs at on-run-end after EVERY `dbt run`, re-ensuring the indexes
    idempotently. CREATE INDEX IF NOT EXISTS is a cheap catalog check when the index
    already exists and only builds (non-CONCURRENTLY, matching the recreate runbook)
    when it is actually missing — so steady-state runs pay almost nothing while a
    post-rebuild gap self-heals. Each table is guarded by to_regclass so a partial
    run / fresh warehouse that lacks a table is skipped, not errored.
...
create index if not exists {{ ix.name }} on {{ ix.rel }} {{ ix.def }};
    {%- endif -%}
  {%- endfor -%}
{%- else -%}
select 1;
